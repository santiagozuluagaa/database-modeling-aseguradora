...
<h1>Agregar VENDEDOR</h1>
    ...
    <br>
    <div class="form-group">
      <label for="repu">Reputación</label>
      <input type="text" name="repu" class="form-control" id="repu">
    </div>
    <br>
...
<!-- CONEXIÓN A LA BASE DE DATOS-->
	...
	<?php
		...
		if(isset($_POST['btn_send_abo'])) // Se verifica si se envió información a través del botón
		{
            ...
            if($consistencia == 0)
            {
            $conexion -> query("INSERT INTO $tabla2(carne, cedula, nombre_completo, salario, nivel_de_educacion, fecha_de_ingreso, tipo_empleado) values ('$carne', '$cedula', '$nom_com', '$salario', '$niv_edu', '$fec_ing', 'Vendedor')");

            $conexion -> query("INSERT INTO $tabla5(carne, reputacion) values ('$carne', '$repu')");
...
<h1>Eliminar VENDEDOR</h1>
    ...
    <input type="submit" value="Eliminar" class="btn btn-success" name="btn_elm_ven">

  <?php

    if(isset($_POST['btn_elm_ven'])) // Se verifica si se presionó el botón
    {
      include("open_conexion.php");
            ...
            $conexion -> query("DELETE FROM $tabla5 where carne = '$carne_eli'"); // Se borra de la tabla de vendedores
