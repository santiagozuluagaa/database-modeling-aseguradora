...
<h1>Agregar ABOGADO</h1>
    ...
    <form method="POST" action="abogado.php" >
    ...
    <br>
    <div class="form-group">
      <label for="campo">Campo de acción</label>
	...
	<input type="submit" value="Enviar" class="btn btn-success" name="btn_send_abo">
...
<!-- CONEXIÓN A LA BASE DE DATOS-->
	...
	<?php
		...
		if(isset($_POST['btn_send_abo'])) // Se verifica si se envió información a través del botón
		{
            ...
            if($consistencia == 0)
            {
            $conexion -> query("INSERT INTO $tabla2(carne, cedula, nombre_completo, salario, nivel_de_educacion, fecha_de_ingreso, tipo_empleado) values ('$carne', '$cedula', '$nom_com', '$salario', '$niv_edu', '$fec_ing', 'Abogado')");

            $conexion -> query("INSERT INTO $tabla4(carne, campo_de_accion,años_de_experiencia) values ('$carne', '$campo' ,'$anos_exp')");
...
<h1>Eliminar ABOGADO</h1>
    ...
    <input type="submit" value="Eliminar" class="btn btn-success" name="btn_elm_abo">

  <?php

    if(isset($_POST['btn_elm_abo'])) // Se verifica si se presionó el botón
    {
      include("open_conexion.php");

            $carne_eli = $_POST['carne_eli']; 
 
            $conexion -> query("DELETE FROM $tabla2 where carne = '$carne_eli'");

            $conexion -> query("DELETE FROM $tabla4 where carne = '$carne_eli'"); // Se borra de la tabla de abogados

            $conexion -> query("DELETE FROM $tabla1 where carne_emp = '$carne_eli'"); // Borrado en cascada
